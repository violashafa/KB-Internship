# Cara Instalasi Uptime Kuma di Sistem Operasi Linux
...
Pada panduan ini, kita akan membahas cara menginstal **Uptime Kuma** pada server Linux menggunakan dua metode instalasi (Non-Docker dan Docker), beserta tiga pilihan database yang tersedia, hingga berhasil login dan sampai ke halaman utama dashboard.
...
Pada panduan ini, Uptime Kuma akan diinstal pada server Linux menggunakan dua metode instalasi yang tersedia, yaitu **Non-Docker (Native)** dan **Docker**.
...
Uptime Kuma dapat dijalankan pada berbagai distribusi Linux, baik menggunakan metode **Docker** maupun **Non-Docker (Native)**. Perbedaan utama antar distribusi hanya terletak pada perintah instalasi paket dependency (Node.js, Git, Docker), sedangkan langkah instalasi Uptime Kuma itu sendiri tetap sama.

| Distribusi                        | Package Manager | Catatan                                                              |
| ---------------------------------- | ---------------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian                    | `apt`            | Digunakan pada panduan ini.                                            |
| CentOS / Rocky Linux / AlmaLinux   | `dnf` / `yum`    | Nama paket dependency seperti `nodejs`, `git`, dan `docker` umumnya tersedia langsung di repository, namun beberapa paket tambahan mungkin memerlukan repository EPEL. |
...
Lanjutkan ke bagian **Pilihan Database** untuk memahami opsi database yang tersedia (Non-Docker hanya menyediakan MariaDB/MySQL dan SQLite).
...
Lanjutkan ke bagian **Pilihan Database** untuk memahami opsi database yang tersedia (Docker menyediakan tiga pilihan: SQLite, Embedded MariaDB, dan MariaDB/MySQL).

---

## 4. Pilihan Database

Pada halaman setup wizard (dibahas di bagian [Akses Website dan Setup Database dengan Akun Admin](#5-akses-website-dan-setup-database-dengan-akun-admin)), Uptime Kuma menyediakan pilihan database. Jumlah pilihan berbeda tergantung metode instalasi:
...
> **Catatan:** Jika MariaDB berada di server yang sama dengan Uptime Kuma, SSL/TLS pada koneksi database tidak perlu diaktifkan karena traffic tidak melewati jaringan (loopback). Apabila Uptime Kuma dijalankan sebagai Docker container sedangkan MariaDB berada di host, gunakan `host.docker.internal` atau IP internal host sebagai Hostname, bukan `localhost`.

---

## 5. Akses Website dan Setup Database dengan Akun Admin
...
2. Pastikan Hostname sesuai konteks instalasi: `localhost` untuk Non-Docker pada server yang sama, atau `host.docker.internal`/IP internal host apabila Uptime Kuma dijalankan via Docker sedangkan MariaDB berada di host.
3. Pastikan user dan privilege sudah dibuat sesuai langkah 4.1, dan Database Name pada wizard sama persis dengan nama database yang dibuat (`kuma`).
4. Pastikan port `3306` dapat diakses dari container/proses Uptime Kuma, terutama apabila MariaDB berada pada server terpisah.

## Kesimpulan

Uptime Kuma dapat diinstal pada sistem operasi Linux baik menggunakan metode Non-Docker (Native dengan Node.js dan PM2) maupun metode Docker (Docker Compose), sesuai kebutuhan dan preferensi environment yang digunakan. Dengan tiga pilihan database yang tersedia (SQLite, Embedded MariaDB, dan MariaDB/MySQL eksternal), instalasi dapat disesuaikan dengan skala penggunaan.
